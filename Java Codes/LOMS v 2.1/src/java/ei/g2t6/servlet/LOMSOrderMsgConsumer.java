...
import ei.g2t6.dao.ConnectionManager;
import java.io.StringReader;
import java.sql.PreparedStatement;
import java.sql.ResultSet;
import java.sql.SQLException;

public class LOMSOrderMsgConsumer
    ...
    public LOMSOrderMsgConsumer(String[] args) {
        parseArgs(args);
    ...
    void run()
        ...
        while (true) {
            ...
            System.out.println("Received message: " + xmlString);

            //process message received (Modified Order)
            DocumentBuilderFactory dbf = DocumentBuilderFactory.newInstance();

            try {
                
                conn = ConnectionManager.getConnection(); //connect to update db
                ...
                Node modOrder = dom.getElementsByTagName("modified_order").item(0);
                
                if (modOrder.hasAttributes()) {
                    
                        Element e = (Element) modOrder;
                        String order = e.getAttribute("order_id");
                        
                        System.out.println("for test: " +order);
                        
                        order_num = Integer.parseInt(order);
                    
                    }

                NodeList nodeList = modOrder.getChildNodes();
                ...
                for (int i = 0; i < nodeList.getLength(); i++) {
                    ...
                    if ("item".equals(node.getNodeName())) {
                        ...
                        if (node.hasAttributes()) {
                            ...
                            //update record with Pending order
                            sql = "UPDATE orderdb set order_status = ? where order_id = ?";
                        ...
                        for (int j = 0; j < itemList.getLength(); j++) {
                            ...
                            if ("confirmed_quantity".equals(itemNode.getNodeName())) {
                                String des = itemNode.getTextContent();
                                int quantity = Integer.parseInt(des);
                                
                                //update record with Pending order
                                sql = "UPDATE order_item set confirm_quantity = ? where order_id = ?";
                                stmt = conn.prepareStatement(sql);
                                ...
                                System.out.println("test Quantity: " + des);
                            }
                        }
